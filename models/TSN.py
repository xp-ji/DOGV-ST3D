...
class TSN(nn.Module):
    def __init__(self, in_channel, num_classes, num_segments=8,
                 base_model='resnet101', new_length=1,
                 consensus_type='avg', before_softmax=True,
                 dropout=0.8,
                 crop_num=1, partial_bn=True, pretrained=True):
...
TSN Configurations:
    ...
    dropout_ratio:      {}
    pretrained:         {}
        """.format(base_model, self.in_channel, self.num_segments, self.new_length,
                   consensus_type, self.dropout, self.pretrained)))
    ...
    def forward(self, input):
        ...
        sample_len = self.in_channel * self.new_length
    ...
    def _construct_depth_model(self, base_model):
        ...
        new_conv = nn.Conv2d(1 * self.new_length, conv_layer.out_channels,
...
if __name__ == "__main__":
    ...
    model = TSN(num_classes=16, in_channel=3)

    print(model)
    output = model(input)

    # print(model.base_model.conv1.weight.data)
